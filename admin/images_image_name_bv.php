...
$images_image_name_blobview = NULL; // Initialize page object first

class cimages_image_name_blobview extends cimages {
	...
	// Page object name
	var $PageObjName = 'images_image_name_blobview';
	...
	function Page_Main() {
		...
		$objBinary = new cUpload('images', 'x_image_name');
		...
		if ($this->Recordset = $conn->Execute($sSql)) {
			if (!$this->Recordset->EOF) {
				if (ob_get_length())
					ob_end_clean();
				$objBinary->Value = $this->Recordset->fields('image_name');
				...
				if (strpos(ew_ServerVar("HTTP_USER_AGENT"), "MSIE") === FALSE)
					header("Content-type: " . ew_ContentType(substr($data, 0, 11), $this->Recordset->fields('image_name')));
				if (trim(strval($this->Recordset->fields('image_name'))) <> "") {
					header("Content-Disposition: attachment; filename=\"" . $this->Recordset->fields('image_name') . "\"");
				}
...
// Create page object
if (!isset($images_image_name_blobview)) $images_image_name_blobview = new cimages_image_name_blobview();

// Page init
$images_image_name_blobview->Page_Init();

// Page main
$images_image_name_blobview->Page_Main();
?>
<?php
$images_image_name_blobview->Page_Terminate();
?>
